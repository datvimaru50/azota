...
// ********* NOTE ******************
// Vì lỗi thư viện multi_image_picker trên Android khi release
// File này sử dụng riêng cho Android
// Sử dụng pagkage image_picker để chụp ảnh trực tiếp từ camera
// Sử dụng thư viện file_picker để chọn ảnh, mp3, mp4 từ thư viện
// *********************************

class SubmitExersiceAndroid extends StatefulWidget {
...
class _SubmitExersiceState extends State<SubmitExersiceAndroid> {
  final picker = ImagePicker();
  List<String> _camImagesPath = []; // Images taken from camera
  ...
  Future<void> shootImage() async {
    ...
    setState(() {
      if (pickedFile != null) {
        _camImagesPath.add(pickedFile.path);
        print(_camImagesPath.toString());
      } else {
  ...
  Future<void> uploadAllImage() async {
    for (var i = 0; i < _camImagesPath.length; i++) {
      await uploadSingleFile(_camImagesPath[i], callbackUpload);
    }
  }
                              ...
                              child: TextButton(
                                ...
                                child: Column(
                                  children: [
                                    ...
                                    Text(
                                      'Chọn file từ thư viện (ảnh, âm thanh, video)',
                                      textAlign: TextAlign.center,
                                      style: TextStyle(
                                        color: Colors.black,
